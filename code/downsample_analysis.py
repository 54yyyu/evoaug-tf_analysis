...
if 'D' in aug_list:
    augment_list.append(augment.RandomDeletionBatch(delete_min=0, delete_max=30))
if 'M' in aug_list:
...
x_train, y_train, x_valid, y_valid, x_test, y_test = utils.H5DataLoader(filepath, downsample=int(downsample))
_, L, A = x_valid.shape
...
for trial in range(num_trials):
 ...
 # reduce learning rate callback
    ...
    model.fit(x_train, y_train,
              ...
              validation_data=(x_valid, y_valid),
              callbacks=[es_callback, reduce_lr]) 
    model.save_weights(os.path.join(output_dir, expt_name+"_finetune_"+downsample+'_'+str(trial)))
    ...
    model.save_weights(os.path.join(output_dir, expt_name+"_standard_"+downsample+'_'+str(trial)))

    # evaluate model
    pred = model.predict(x_test, batch_size=512)
    aug_results = utils.evaluate_model(y_test, pred)
    trial_standard_results.append(aug_results)
...
# save results
with open(os.path.join(output_dir, expt_name+'_'+downsample+'_results.pickle'), 'wb') as fout:
